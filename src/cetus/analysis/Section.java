...
public class Section extends ArrayList<Section.ELEMENT> implements Cloneable {
    ...
    */
    ...
    private static Expression unionBoundStride1(RangeExpression re1, RangeExpression re2, RangeDomain rd) {
    ...
    */
    private static Expression
        ...
        if(!(re1.getStride() instanceof IntegerLiteral)){
            ...
            re1.setStride(new IntegerLiteral(1));
        }
        if(!(re2.getStride() instanceof IntegerLiteral)){
        ...
        //        (((IntegerLiteral)re2.getStride()).getValue() == 1)
        //) {
            //Both RangeExpressions' stride are equal to 1
            return unionBoundStride1(re1, re2, rd);
        //}
        //else
        //{
        //    throw new IllegalArgumentException("Unexpected Stride re1: " + re1.toString() + " re2: " + re2.toString());
        //}
    }

    private static Expression intersectBoundStride1(RangeExpression re1, RangeExpression re2, RangeDomain rd) {
        Expression lb = null, ub = null;
        Relation rel = rd.compare(re1.getLB(), re2.getLB());
        if (rel.isGE()) {
            lb = re1.getLB();
    ...
    private static long LCM(long a, long b) { return Math.abs(a*b) / GCD(a,b);}
    ...
    */
    private static Expression
        ...
        //if(
        //        (re1.getStride() instanceof IntegerLiteral) &&
        //        (re2.getStride() instanceof IntegerLiteral) &&
        //        (((IntegerLiteral)re1.getStride()).getValue() == 1) &&
        //        (((IntegerLiteral)re2.getStride()).getValue() == 1)
        //) {
            //Both RangeExpressions' stride are equal to 1
        //    return intersectBoundStride1(re1, re2, rd);
        //}
        //else
        if(
            (re1.getStride() instanceof IntegerLiteral) &&
            (re2.getStride() instanceof IntegerLiteral) &&
            (re1.getLB() instanceof IntegerLiteral) &&
            (re2.getLB() instanceof IntegerLiteral) &&
            (re1.getUB() instanceof IntegerLiteral) &&
            (re2.getUB() instanceof IntegerLiteral)
                ){
            long lb1 = ((IntegerLiteral)re1.getLB()).getValue();
            long lb2 = ((IntegerLiteral)re2.getLB()).getValue();
            long newLb = (lb1 > lb2)?lb1:lb2;
            long ub1 = ((IntegerLiteral)re1.getUB()).getValue();
            long ub2 = ((IntegerLiteral)re2.getUB()).getValue();
            long newUb = (ub1 < ub2)?ub1:ub2;
            long s1 = ((IntegerLiteral)re1.getStride()).getValue();
            long s2 = ((IntegerLiteral)re2.getStride()).getValue();
            long c = lb2 - lb1;

            if(c % GCD(s1,-s2) != 0)
                return null;

            long[] r = extended_gcd(s1, -s2);

            long x0 = lb1 + s1*(r[1]*c)/r[0];
            long newStride = LCM(s1, s2);
            ...
            newLb = x0;
            newUb = newLb + ((newUb - newLb)/newStride) * newStride;

            RangeExpression result = new RangeExpression(new IntegerLiteral(newLb), new IntegerLiteral(newUb), new IntegerLiteral(newStride));
