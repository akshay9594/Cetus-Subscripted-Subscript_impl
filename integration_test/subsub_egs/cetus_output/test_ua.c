...
*/
/*
Counter that is monotonically increasing (by less than 231 per
   increment), with a single writer, and an arbitrary number of
   readers. 
*/
struct named_test_ua_c_17940
{
	unsigned int __low;
	unsigned int __high;
};

union named_test_ua_c_17925
{
	unsigned long long int __value64;
	struct named_test_ua_c_17940 __value32;
};

typedef union named_test_ua_c_17925 __atomic_wide_counter;
...
struct named_test_ua_c_18237
{
	int __data;
};

typedef struct named_test_ua_c_18237 __once_flag;
/*
...
*/
union named_test_ua_c_18262
{
	char __size[4];
	int __align;
};

typedef union named_test_ua_c_18262 pthread_mutexattr_t;
/*
Data structure for condition variable handling.  The structure of
   the attribute type is not exposed on purpose. 
*/
union named_test_ua_c_18284
{
	char __size[4];
	int __align;
};

typedef union named_test_ua_c_18284 pthread_condattr_t;
...
typedef union named_test_ua_c_18353 pthread_mutex_t;
union named_test_ua_c_18384
{
...
typedef union named_test_ua_c_18384 pthread_cond_t;
/*
Data structure for reader-writer lock variable handling.  The
   structure of the attribute type is deliberately not exposed. 
*/
union named_test_ua_c_18418
{
...
typedef union named_test_ua_c_18418 pthread_rwlock_t;
union named_test_ua_c_18449
{
	char __size[8];
	long int __align;
};

typedef union named_test_ua_c_18449 pthread_rwlockattr_t;
...
*/
union named_test_ua_c_18482
{
	char __size[32];
	long int __align;
};

typedef union named_test_ua_c_18482 pthread_barrier_t;
union named_test_ua_c_18506
{
	char __size[4];
	int __align;
};

typedef union named_test_ua_c_18506 pthread_barrierattr_t;
/*
...
int main()
{
	...
	#pragma cetus private(col, col_0, col_1, i, i_0, i_1, i_10, i_11, i_12, i_2, i_3, i_4, i_5, i_6, i_7, i_8, i_9, ie, iface, ig, ig1, ig2, ig3, ig4, ije1, ije2, il, il1, il2, il3, il4, j, nnje, x, y, z) 
	...
	for (ie=0; ie<nelt; ie ++ )
	{
		...
		for (iface=0; iface<6; iface ++ )
		{
			...
			if (nnje==2)
			{
				...
				#pragma cetus parallel 
				/*
				Disabled due to low profitability: #pragma omp parallel for private(x, y, z)
				*/
				for (x=0; x<nnje; x ++ )
				{
				...
				#pragma cetus parallel 
				#pragma omp parallel for if((10000<(1L+(799L*nnje)))) private(col, i, i_0, ig, ije1, ije2, il, j)
				...
				#pragma omp parallel if((10000<(52905L+(316L*nnje)))) private(col, col_0, i, i_1, i_2, i_3, ije1, il, j)
				{
					double * reduce = (double * )malloc(8800*sizeof (double));
					int reduce_span_0;
					for (reduce_span_0=0; reduce_span_0<8800; reduce_span_0 ++ )
					{
					...
					for (ije1=0; ije1<nnje; ije1 ++ )
					{
						...
						for (i_1=0; i_1<=2; i_1 ++ )
						{
							...
							for (j=0; j<5; j ++ )
							{
								reduce[il]=(reduce[il]+((qbnew[ije1][j][(1+i_1)-1]*tmp[ije1][j][col])*0.5));
							}
						}
						i=(1+i_1);
						/* for column 1 ~ lx-2 */
						...
						for (col_0=0; col_0<=2; col_0 ++ )
						{
							...
							reduce[il]=(reduce[il]+(tmp[ije1][i][1+col_0]*0.5));
							...
							for (i_2=0; i_2<=2; i_2 ++ )
							{
								...
								for (j=0; j<5; j ++ )
								{
									reduce[il]=(reduce[il]+(qbnew[ije1][j][(1+i_2)-1]*tmp[ije1][j][1+col_0]));
								}
							}
							i=(1+i_2);
						}
						...
						for (i_3=0; i_3<=2; i_3 ++ )
						{
							...
							for (j=0; j<5; j ++ )
							{
								reduce[il]=(reduce[il]+((qbnew[ije1][j][(1+i_3)-1]*tmp[ije1][j][col])*0.5));
							}
						}
						i=(1+i_3);
					}
					...
					#pragma omp critical
					{
						for (reduce_span_0=0; reduce_span_0<8800; reduce_span_0 ++ )
						{
							tx[reduce_span_0]+=reduce[reduce_span_0];
						}
					}
				}
			...
			else
			{
				...
				#pragma cetus parallel 
				/*
				Disabled due to low profitability: #pragma omp parallel for private(i, i_4, ig, il) lastprivate(col_1)
				*/
				...
				if (idmo[ie][iface][0][0][0][5-1]!=( - 1))
				{
					...
					#pragma omp parallel private(ig, ije1, il, j)
					{
						double * reduce = (double * )malloc(8800*sizeof (double));
						int reduce_span_0;
						for (reduce_span_0=0; reduce_span_0<8800; reduce_span_0 ++ )
						...
						#pragma cetus for  
						#pragma omp for lastprivate(i_5)
						for (i_5=0; i_5<=2; i_5 ++ )
						{
							...
							for (ije1=0; ije1<2; ije1 ++ )
							{
								...
								for (j=0; j<5; j ++ )
								{
									...
									reduce[il]=(reduce[il]+((qbnew[ije1][j][(1+i_5)-1]*tmor[ig])*0.5));
								}
							}
						}
						#pragma cetus critical  
				...
				else
				{
					...
					#pragma cetus parallel 
					/*
					Disabled due to low profitability: #pragma omp parallel for private(ig, il) lastprivate(i_6)
					*/
					for (i_6=0; i_6<=2; i_6 ++ )
					{
				...
				if (idmo[ie][iface][1][0][1][5-1]!=( - 1))
				{
					...
					#pragma omp parallel private(ig, ije1, il, j)
					{
						...
						#pragma cetus for  
						#pragma omp for lastprivate(i_7)
						for (i_7=0; i_7<=2; i_7 ++ )
						{
							...
							for (ije1=0; ije1<2; ije1 ++ )
							{
								...
								for (j=0; j<5; j ++ )
								{
									...
									reduce[il]=(reduce[il]+((qbnew[ije1][j][(1+i_7)-1]*tmor[ig])*0.5));
								}
							}
						}
						#pragma cetus critical  
				...
				else
				{
					...
					#pragma cetus parallel 
					/*
					Disabled due to low profitability: #pragma omp parallel for private(ig, il) lastprivate(i_8)
					*/
					for (i_8=0; i_8<=2; i_8 ++ )
					{
				...
				if (idmo[ie][iface][0][1][5-1][1]!=( - 1))
				{
					...
					#pragma omp parallel private(ig, ije1, il, j)
					{
						...
						#pragma cetus for  
						#pragma omp for lastprivate(i_9)
						for (i_9=0; i_9<=2; i_9 ++ )
						{
							...
							for (ije1=0; ije1<2; ije1 ++ )
							{
								...
								for (j=0; j<5; j ++ )
								{
									...
									reduce[il]=(reduce[il]+((qbnew[ije1][j][(1+i_9)-1]*tmor[ig])*0.5));
								}
							}
						}
						#pragma cetus critical  
				...
				else
				{
					...
					#pragma cetus parallel 
					/*
					Disabled due to low profitability: #pragma omp parallel for private(ig, il) lastprivate(i_10)
					*/
				...
				if (idmo[ie][iface][0][0][5-1][0]!=( - 1))
				{
					...
					#pragma omp parallel private(ig, ije1, il, j)
					{
						...
						#pragma cetus for  
						#pragma omp for lastprivate(i_11)
						for (i_11=0; i_11<=2; i_11 ++ )
						{
							...
							for (ije1=0; ije1<2; ije1 ++ )
							{
								...
								for (j=0; j<5; j ++ )
								{
									...
									reduce[il]=(reduce[il]+((qbnew[ije1][j][(1+i_11)-1]*tmor[ig])*0.5));
								}
							}
						}
						#pragma cetus critical  
				...
				else
				{
					...
					#pragma cetus parallel 
					/*
					Disabled due to low profitability: #pragma omp parallel for private(ig, il) lastprivate(i_12)
					*/
