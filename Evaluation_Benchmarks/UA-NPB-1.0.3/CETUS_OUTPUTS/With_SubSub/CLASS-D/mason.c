...
void mortar()
{
	...
	#pragma cetus private(cb, cb1, cb2, iel, ij1, ij2, ntemp, ntemp1, sumcb) 
	#pragma loop name mortar#0 
	#pragma cetus parallel 
	#pragma omp parallel for if((10000<(1L+(480L*nelt)))) private(cb, cb1, cb2, iel, ij1, ij2, ntemp, ntemp1, sumcb)
		...
		else
		{
			...
			else
			{
				...
				else
				{
					if (sumcb==5)
					{
						/* the three face types for face 1,3 and 5 are 23 0 0 (not  */
						/* necessarily in this order) */
						newc[iel]=4;
						if ((cb==2)||(cb==3))
						{
	...
	#pragma cetus lastprivate(count) 
	#pragma loop name mortar#1 
	for (iel=0; iel<nelt; iel ++ )
	{
	...
	#pragma cetus private(cb1, cb2, cb3, cb4, cb5, cb6, iel, ntemp) 
	#pragma loop name mortar#2 
	for (iel=0; iel<nelt; iel ++ )
	{
		newc[iel]=0;
		newe[iel]=0;
	...
	#pragma cetus private(cb, cb1, edge_g, face2, i, ie, ie2, iel, ii, ii_0, ii_1, jface, jj, jj_0, jj_1, ne, ntemp, space) 
	#pragma loop name mortar#3 
	for (iel=0; iel<nelt; iel ++ )
	{
		front[iel]=(front[iel]-newc[iel]);
		count=(nvertex+front[iel]);
		#pragma cetus private(cb, cb1, edge_g, face2, i, ie, ie2, ii, ii_0, ii_1, jface, jj, jj_0, jj_1, ne, ntemp, space) 
		#pragma loop name mortar#3#0 
		for (i=0; i<6; i ++ )
		{
			cb1=cbc[iel][i];
			if (i<2)
			...
			#pragma cetus private(edge_g, face2, ie, ie2, ntemp) 
			#pragma loop name mortar#3#0#0 
			...
			if (fassign[iel][i])
			{
				...
				{
					/* Normalized Loop */
					#pragma cetus private(ii, ii_0) 
					#pragma cetus lastprivate(jj_0) 
					#pragma loop name mortar#3#0#1 
					for (jj_0=0; jj_0<=2; jj_0 ++ )
					{
						/* Normalized Loop */
						#pragma cetus lastprivate(ii_0) 
						#pragma loop name mortar#3#0#1#0 
						for (ii_0=0; ii_0<=2; ii_0 ++ )
						{
							idmo[iel][i][0][0][1+jj_0][1+ii_0]=count;
							count=(count+1);
						}
						ii=(1+ii_0);
					}
				...
				else
				{
					if (cb==2)
					{
						if (idmo[iel][i][0][0][1][1]==( - 1))
						{
							...
							jface=jjface[i];
							/* Normalized Loop */
							#pragma cetus private(ii, ii_1) 
							#pragma cetus lastprivate(jj_1) 
							#pragma loop name mortar#3#0#2 
							for (jj_1=0; jj_1<=2; jj_1 ++ )
							{
								/* Normalized Loop */
								#pragma cetus lastprivate(ii_1) 
								#pragma loop name mortar#3#0#2#0 
								for (ii_1=0; ii_1<=2; ii_1 ++ )
								{
									idmo[iel][i][0][0][1+jj_1][1+ii_1]=count;
									idmo[ntemp][jface][0][0][1+jj_1][1+ii_1]=count;
									count=(count+1);
								}
								ii=(1+ii_1);
							}
	...
	/* from neighbors. */
	#pragma cetus private(cb, i, iel, ii, iii, jface, jj, jjj, ntemp) 
	#pragma loop name mortar#4 
	for (iel=0; iel<nelt; iel ++ )
	{
		#pragma cetus private(cb, i, ii, iii, jface, jj, jjj, ntemp) 
		#pragma loop name mortar#4#0 
		for (i=0; i<6; i ++ )
		{
			cb=cbc[iel][i];
			if (cb==3)
			...
			if (cb==3)
			{
				#pragma cetus private(ii, iii, jj, jjj, ntemp) 
				#pragma loop name mortar#4#0#0 
				for (iii=0; iii<2; iii ++ )
				{
					#pragma cetus private(ii, jj, jjj, ntemp) 
					#pragma loop name mortar#4#0#0#0 
					for (jjj=0; jjj<2; jjj ++ )
					{
						ntemp=sje[iel][i][jjj][iii];
						#pragma cetus private(ii, jj) 
						#pragma loop name mortar#4#0#0#0#0 
						#pragma cetus parallel 
						/*
						...
						*/
						for (jj=0; jj<5; jj ++ )
						{
							...
							#pragma loop name mortar#4#0#0#0#0#0 
							for (ii=0; ii<5; ii ++ )
							{
...
static void mor_edge(int ie, int face, int iel, int mor_v[3])
{
	...
	if (ie==0)
	{
		j=0;
		/* Normalized Loop */
		#pragma cetus lastprivate(nn_0) 
		#pragma loop name mor_edge#0 
		#pragma cetus parallel 
		/*
		Disabled due to low profitability: #pragma omp parallel for lastprivate(nn_0)
		*/
		for (nn_0=0; nn_0<=2; nn_0 ++ )
		{
			idmo[iel][face][0][0][j][1+nn_0]=mor_v[(1+nn_0)-1];
		}
		nn=(1+nn_0);
	}
	else
	{
		if (ie==1)
		{
			i=(5-1);
			/* Normalized Loop */
			#pragma cetus lastprivate(nn_1) 
			#pragma loop name mor_edge#1 
			#pragma cetus parallel 
			/*
			Disabled due to low profitability: #pragma omp parallel for lastprivate(nn_1)
			*/
			for (nn_1=0; nn_1<=2; nn_1 ++ )
			{
				idmo[iel][face][0][0][1+nn_1][i]=mor_v[(1+nn_1)-1];
			}
			nn=(1+nn_1);
		}
		else
		{
			if (ie==2)
			{
				j=(5-1);
				/* Normalized Loop */
				#pragma cetus lastprivate(nn_2) 
				#pragma loop name mor_edge#2 
				#pragma cetus parallel 
				/*
				Disabled due to low profitability: #pragma omp parallel for lastprivate(nn_2)
				*/
				for (nn_2=0; nn_2<=2; nn_2 ++ )
				{
					idmo[iel][face][0][0][j][1+nn_2]=mor_v[(1+nn_2)-1];
				}
				nn=(1+nn_2);
			}
			else
			{
				if (ie==3)
				{
					i=0;
					/* Normalized Loop */
					#pragma cetus lastprivate(nn_3) 
					#pragma loop name mor_edge#3 
					#pragma cetus parallel 
					/*
					Disabled due to low profitability: #pragma omp parallel for lastprivate(nn_3)
					*/
					for (nn_3=0; nn_3<=2; nn_3 ++ )
					{
						idmo[iel][face][0][0][1+nn_3][i]=mor_v[(1+nn_3)-1];
					}
					nn=(1+nn_3);
				}
			}
...
static void edgecopy_s(int face, int iel)
{
	...
	*/
	for (i_0=0; i_0<=2; i_0 ++ )
	{
		mor_s_v[0][(1+i_0)-1]=idmo[ntemp1][jjface[face]][0][0][0][1+i_0];
	}
	i=(1+i_0);
	mor_s_v[0][5-2]=idmo[ntemp1][jjface[face]][1][0][0][5-1];
	#pragma cetus private(i) 
	#pragma loop name edgecopy_s#1 
	#pragma cetus parallel 
	/*
	...
	*/
	for (i_1=0; i_1<=2; i_1 ++ )
	{
		mor_s_v[0][(1+i_1)-1]=idmo[ntemp2][jjface[face]][0][0][1+i_1][5-1];
	}
	i=(1+i_1);
	...
	*/
	for (i_2=0; i_2<=2; i_2 ++ )
	{
		mor_s_v[1][1+i_2]=idmo[ntemp4][jjface[face]][0][0][1+i_2][5-1];
	}
	i=(1+i_2);
	...
	*/
	for (i_3=0; i_3<=2; i_3 ++ )
	{
		mor_s_v[0][(1+i_3)-1]=idmo[ntemp3][jjface[face]][0][0][5-1][1+i_3];
	}
	i=(1+i_3);
	...
	#pragma loop name edgecopy_s#5 
	#pragma cetus parallel 
	/*
	Disabled due to low profitability: #pragma omp parallel for lastprivate(i_4)
	*/
	for (i_4=0; i_4<=2; i_4 ++ )
	{
		mor_s_v[1][1+i_4]=idmo[ntemp4][jjface[face]][0][0][5-1][1+i_4];
	}
	i=(1+i_4);
	...
	*/
	for (i_5=0; i_5<=2; i_5 ++ )
	{
		mor_s_v[0][(1+i_5)-1]=idmo[ntemp1][jjface[face]][0][0][1+i_5][0];
	}
	i=(1+i_5);
	mor_s_v[0][5-2]=idmo[ntemp1][jjface[face]][0][1][5-1][0];
	#pragma cetus private(i) 
	#pragma loop name edgecopy_s#7 
	#pragma cetus parallel 
	/*
	Disabled due to low profitability: #pragma omp parallel for private(i)
	*/
...
static void mor_s_e(int n, int face, int iel, int mor_s_v[2][4])
{
	...
	if (n==0)
	{
		...
		*/
		for (i_0=0; i_0<=3; i_0 ++ )
		{
			idmo[iel][face][0][0][0][1+i_0]=mor_s_v[0][(1+i_0)-1];
		}
		i=(1+i_0);
		#pragma cetus private(i) 
		#pragma loop name mor_s_e#1 
		#pragma cetus parallel 
		/*
		Disabled due to low profitability: #pragma omp parallel for private(i)
		*/
	...
	else
	{
		if (n==1)
		{
			...
			*/
			for (i_1=0; i_1<=3; i_1 ++ )
			{
				idmo[iel][face][1][0][1+i_1][5-1]=mor_s_v[0][(1+i_1)-1];
			}
			i=(1+i_1);
			#pragma cetus private(i) 
			#pragma loop name mor_s_e#3 
			#pragma cetus parallel 
			/*
			Disabled due to low profitability: #pragma omp parallel for private(i)
			*/
		...
		else
		{
			if (n==2)
			{
				...
				*/
				for (i_2=0; i_2<=3; i_2 ++ )
				{
					idmo[iel][face][0][1][5-1][1+i_2]=mor_s_v[0][(1+i_2)-1];
				}
				i=(1+i_2);
				#pragma cetus private(i) 
				#pragma loop name mor_s_e#5 
				#pragma cetus parallel 
				/*
				Disabled due to low profitability: #pragma omp parallel for private(i)
				*/
			...
			else
			{
				if (n==3)
				{
					...
					*/
					for (i_3=0; i_3<=3; i_3 ++ )
					{
						idmo[iel][face][0][0][1+i_3][0]=mor_s_v[0][(1+i_3)-1];
					}
					i=(1+i_3);
					#pragma cetus private(i) 
					#pragma loop name mor_s_e#7 
					#pragma cetus parallel 
					/*
					Disabled due to low profitability: #pragma omp parallel for private(i)
					*/
...
/* which indicates that mor_s_v  corresponds to leftbottom or  */
/* righttop part of the edge. */
/* ------------------------------------------------------------ */
static void mor_s_e_nn(int n, int face, int iel, int mor_s_v[4], int nn)
{
	int i;
	...
	if (n==0)
	{
		if (nn==0)
		{
			...
			*/
			for (i_0=0; i_0<=3; i_0 ++ )
			{
				idmo[iel][face][0][0][0][1+i_0]=mor_s_v[(1+i_0)-1];
			}
			i=(1+i_0);
		}
		else
		...
		if (n==1)
		{
			if (nn==0)
			{
				...
				*/
				for (i_1=0; i_1<=3; i_1 ++ )
				{
					idmo[iel][face][1][0][1+i_1][5-1]=mor_s_v[(1+i_1)-1];
				}
				i=(1+i_1);
			}
			else
			...
			if (n==2)
			{
				if (nn==0)
				{
					...
					*/
					for (i_2=0; i_2<=3; i_2 ++ )
					{
						idmo[iel][face][0][1][5-1][1+i_2]=mor_s_v[(1+i_2)-1];
					}
					i=(1+i_2);
				}
				else
				...
				if (n==3)
				{
					if (nn==0)
					{
						...
						*/
						for (i_3=0; i_3<=3; i_3 ++ )
						{
							idmo[iel][face][0][0][1+i_3][0]=mor_s_v[(1+i_3)-1];
						}
						i=(1+i_3);
					}
					else
...
static void mortar_vertex(int i, int iel, int count)
{
	...
	#pragma loop name mortar_vertex#0 
	#pragma cetus parallel 
	/*
	...
	*/
	...
	/* face in face_a. */
	#pragma cetus private(l) 
	#pragma loop name mortar_vertex#1 
	#pragma cetus parallel 
	/*
	Disabled due to low profitability: #pragma omp parallel for private(l)
	*/
	...
	#pragma cetus private(ii, itemp, jj, ntemp) 
	#pragma loop name mortar_vertex#2 
	...
	#pragma cetus private(ii, itemp, jj, l, lc, nbe, ntemp, temp) 
	#pragma loop name mortar_vertex#3 
	for (l=0; l<3; l ++ )
	{
...
/* corresponds to the leftbottom or right/top part of the edge */
/* on iel. */
/* --------------------------------------------------------------- */
static void mor_ne(int mor_v[3], int nn, int edge, int face, int edge2, int face2, int ntemp, int iel)
{
			...
			else
			{
				if (edge==1)
				{
					if (nn==0)
					{
						...
						#pragma loop name mor_ne#6 
						#pragma cetus parallel 
						/*
						Disabled due to low profitability: #pragma omp parallel for lastprivate(i_6)
						*/
						for (i_6=0; i_6<=2; i_6 ++ )
						{
							mor_s_v[(1+i_6)-1]=mor_v[(1+i_6)-1];
						}
						i=(1+i_6);
						mor_s_v[3]=idmo[ntemp][face][1][1][5-1][5-1];
					}
