...
void adaptation(logical * ifmortar, int step)
{
	...
	/*                morton index */
	#pragma cetus private(iel, miel) 
	#pragma loop name adaptation#0 
...
static void do_coarsen(logical * if_coarsen, int * icoarsen, int neltold)
{
	...
	#pragma cetus private(i, ic, iel, miel, parent) 
	#pragma loop name do_coarsen#0 
	for (miel=0; miel<nelt; miel ++ )
	{
		ifcoa[miel]=false;
		front[miel]=0;
		...
		if (ich[iel]==2)
		{
			...
			if ((( ! (ic&(1<<0)))&&( ! (ic&(1<<1))))&&( ! (ic&(1<<2))))
			{
				...
				#pragma loop name do_coarsen#0#0 
				for (i=0; i<8; i ++ )
				{
				...
				if (test1)
				{
					test2=true;
					#pragma cetus private(i) 
					#pragma loop name do_coarsen#0#1 
					for (i=0; i<8; i ++ )
					{
						if (ich[ntp[i]]!=2)
						{
					...
					if (test2)
					{
						test3=true;
						#pragma cetus private(i) 
						#pragma loop name do_coarsen#0#2 
						for (i=0; i<8; i ++ )
						{
							if ( ! icheck(ntp[i], i))
							{
				...
				if (test)
				{
					ifcoa[miel]=true;
					#pragma cetus private(i) 
					#pragma loop name do_coarsen#0#3 
					for (i=0; i<8; i ++ )
					{
						ifcoa_id[ntp[i]]=true;
					}
					front[miel]=1;
					#pragma cetus private(i) 
					#pragma loop name do_coarsen#0#4 
					for (i=0; i<7; i ++ )
					{
						skip[ntp[i+1]]=true;
					}
	...
	#pragma cetus private(iel, miel, mielnew) 
	#pragma loop name do_coarsen#1 
	for (miel=0; miel<nelt; miel ++ )
	{
		iel=mt_to_id_old[miel];
		if ( ! skip[iel])
	...
	#pragma cetus private(iel, index, miel) 
	#pragma loop name do_coarsen#2 
...
static void do_refine(logical * ifmortar, int * irefine)
{
	...
	#pragma cetus private(miel) 
	#pragma loop name do_refine#0 
	#pragma cetus parallel 
	#pragma omp parallel for if((10000<(1L+(5L*nelt)))) private(miel)
	for (miel=0; miel<nelt; miel ++ )
	{
	...
	#pragma cetus private(iel, miel, mielnew, ntemp) 
	#pragma loop name do_refine#2 
	for (miel=0; miel<nelt; miel ++ )
	{
		iel=mt_to_id_old[miel];
		if (ich[iel]==4)
	...
	#pragma cetus private(cb, facedir, i, i_0, i_1, i_2, i_3, i_4, i_5, iel, index, j, j_0, j_1, j_2, j_3, jface, k, le, miel, mielnew, n1, n2, ndir, ne, ntemp, treetemp, xhalf, xleft, xright, yhalf, yleft, yright, zhalf, zleft, zright) 
	#pragma loop name do_refine#3 
	for (index=0; index<num_refine; index ++ )
	{
		...
		#pragma loop name do_refine#3#0 
		for (j=0; j<7; j ++ )
		{
		...
		tree[iel]=ntemp;
		#pragma cetus private(i) 
		#pragma loop name do_refine#3#1 
		#pragma cetus parallel 
		/*
		...
		*/
		...
		#pragma loop name do_refine#3#2 
		#pragma cetus parallel 
		/*
		Disabled due to low profitability: #pragma omp parallel for private(i, i_0) lastprivate(j_0)
		*/
		for (j_0=0; j_0<=3; j_0 ++ )
		{
			/* Normalized Loop */
			#pragma cetus lastprivate(i_0) 
			#pragma loop name do_refine#3#2#0 
			for (i_0=0; i_0<=3; i_0 ++ )
			{
				xc[nelt+(2*j_0)][2*i_0]=xhalf;
				xc[nelt+(2*j_0)][(2*i_0)+1]=xright;
			}
			i=(2*i_0);
		}
		j=(2*j_0);
		...
		#pragma loop name do_refine#3#3 
		#pragma cetus parallel 
		/*
		Disabled due to low profitability: #pragma omp parallel for private(i, i_1) lastprivate(j_1)
		*/
		for (j_1=0; j_1<=2; j_1 ++ )
		{
			/* Normalized Loop */
			#pragma cetus lastprivate(i_1) 
			#pragma loop name do_refine#3#3#0 
			for (i_1=0; i_1<=3; i_1 ++ )
			{
				xc[nelt+(1+(2*j_1))][2*i_1]=xleft;
				xc[nelt+(1+(2*j_1))][(2*i_1)+1]=xhalf;
			}
			i=(2*i_1);
		}
		j=(1+(2*j_1));
		...
		#pragma loop name do_refine#3#4 
		#pragma cetus parallel 
		/*
		Disabled due to low profitability: #pragma omp parallel for lastprivate(i_2)
		*/
		for (i_2=0; i_2<=3; i_2 ++ )
		{
			xc[iel][2*i_2]=xleft;
			xc[iel][(2*i_2)+1]=xhalf;
		}
		i=(2*i_2);
		#pragma cetus private(i) 
		#pragma loop name do_refine#3#5 
		#pragma cetus parallel 
		/*
		Disabled due to low profitability: #pragma omp parallel for private(i)
		*/
		...
		#pragma loop name do_refine#3#6 
		#pragma cetus parallel 
		/*
		Disabled due to low profitability: #pragma omp parallel for lastprivate(i_3)
		*/
		for (i_3=0; i_3<=1; i_3 ++ )
		{
			yc[nelt+0][2+i_3]=yhalf;
			yc[nelt+3][2+i_3]=yhalf;
			yc[nelt+4][2+i_3]=yhalf;
			yc[nelt+0][(2+i_3)+4]=yhalf;
			yc[nelt+3][(2+i_3)+4]=yhalf;
			yc[nelt+4][(2+i_3)+4]=yhalf;
		}
		i=(2+i_3);
		/* Normalized Loop */
		#pragma cetus private(i, i_4) 
		#pragma cetus lastprivate(j_2) 
		#pragma loop name do_refine#3#7 
		#pragma cetus parallel 
		/*
		Disabled due to low profitability: #pragma omp parallel for private(i, i_4) lastprivate(j_2)
		*/
		for (j_2=0; j_2<=1; j_2 ++ )
		{
			#pragma cetus private(i) 
			#pragma loop name do_refine#3#7#0 
			for (i=0; i<2; i ++ )
			{
				yc[nelt+(1+j_2)][i]=yhalf;
				yc[(nelt+(1+j_2))+4][i]=yhalf;
				yc[nelt+(1+j_2)][i+4]=yhalf;
				yc[(nelt+(1+j_2))+4][i+4]=yhalf;
			}
			/* Normalized Loop */
			#pragma cetus lastprivate(i_4) 
			#pragma loop name do_refine#3#7#1 
			for (i_4=0; i_4<=1; i_4 ++ )
			{
				yc[nelt+(1+j_2)][2+i_4]=yright;
				yc[(nelt+(1+j_2))+4][2+i_4]=yright;
				yc[nelt+(1+j_2)][(2+i_4)+4]=yright;
				yc[(nelt+(1+j_2))+4][(2+i_4)+4]=yright;
			}
			i=(2+i_4);
		}
		j=(1+j_2);
		#pragma cetus private(i) 
		#pragma loop name do_refine#3#8 
		#pragma cetus parallel 
		/*
		Disabled due to low profitability: #pragma omp parallel for private(i)
		*/
		...
		#pragma loop name do_refine#3#9 
		#pragma cetus parallel 
		/*
		...
		*/
		...
		{
			yc[iel][2+i_5]=yhalf;
			yc[iel][(2+i_5)+4]=yhalf;
		}
		i=(2+i_5);
		#pragma cetus private(i, j) 
		#pragma loop name do_refine#3#10 
		#pragma cetus parallel 
		/*
		...
		*/
		...
		{
			#pragma cetus private(i) 
			#pragma loop name do_refine#3#10#0 
			for (i=0; i<4; i ++ )
			{
				zc[nelt+j][i]=zleft;
				zc[nelt+j][i+4]=zhalf;
		...
		#pragma loop name do_refine#3#11 
		#pragma cetus parallel 
		/*
		Disabled due to low profitability: #pragma omp parallel for private(i) lastprivate(j_3)
		*/
		for (j_3=0; j_3<=3; j_3 ++ )
		{
			#pragma cetus private(i) 
			#pragma loop name do_refine#3#11#0 
			for (i=0; i<4; i ++ )
			{
				zc[nelt+(3+j_3)][i]=zhalf;
				zc[nelt+(3+j_3)][i+4]=zright;
			}
		}
		j=(3+j_3);
		#pragma cetus private(i) 
		#pragma loop name do_refine#3#12 
		#pragma cetus parallel 
		/*
		Disabled due to low profitability: #pragma omp parallel for private(i)
		*/
		...
		#pragma cetus firstprivate(sje) 
		#pragma cetus private(cb, facedir, i, jface, k, n1, n2, ndir) 
		#pragma cetus lastprivate(le, ne, sje) 
		#pragma loop name do_refine#3#13 
		for (ndir=0; ndir<3; ndir ++ )
		{
			#pragma cetus firstprivate(sje) 
			#pragma cetus private(cb, facedir, i, jface, k, n1, n2) 
			#pragma cetus lastprivate(le, ne, sje) 
			#pragma loop name do_refine#3#13#0 
			for (facedir=0; facedir<=1; facedir ++ )
			{
				...
				#pragma loop name do_refine#3#13#0#0 
				#pragma cetus parallel 
				/*
				...
				*/
				...
				/* face of the parent element */
				#pragma cetus private(k) 
				#pragma loop name do_refine#3#13#0#1 
				for (k=0; k<4; k ++ )
				{
					cbc[le[k]][i]=2;
					sje[le[k]][i][0][0]=ne[k];
				...
				if (cb==2)
				{
					...
					if (ich[ntemp]!=4)
					{
						...
						#pragma cetus firstprivate(sje) 
						#pragma cetus private(k) 
						#pragma cetus lastprivate(sje) 
						#pragma loop name do_refine#3#13#0#2 
						for (k=0; k<4; k ++ )
						{
							cbc[ne[k]][i]=1;
					...
					else
					{
						...
						#pragma cetus private(k, n2) 
						#pragma loop name do_refine#3#13#0#3 
						for (k=0; k<4; k ++ )
						{
							cbc[ne[k]][i]=2;
				...
				else
				{
					...
					{
						#pragma cetus private(k) 
						#pragma loop name do_refine#3#13#0#4 
						for (k=0; k<4; k ++ )
						{
							cbc[ne[k]][i]=2;
							if (k==0)
					...
					else
					{
						...
						{
							#pragma cetus private(k) 
							#pragma loop name do_refine#3#13#0#5 
							#pragma cetus parallel 
							/*
							Disabled due to low profitability: #pragma omp parallel for private(k)
							*/
...
static void find_coarsen(logical * if_coarsen, int neltold)
{
	...
	int iel, i;
	( * if_coarsen)=false;
	#pragma cetus private(i, iel) 
	#pragma loop name find_coarsen#0 
	for (iel=0; iel<neltold; iel ++ )
	{
		if ( ! skip[iel])
		{
			...
			if ( ! iftouch(iel))
			{
				iftemp=false;
				#pragma cetus private(i) 
				#pragma loop name find_coarsen#0#0 
				for (i=0; i<6; i ++ )
				{
...
static void find_refine(logical * if_refine)
{
	...
	#pragma loop name find_refine#0 
...
static void check_refine(logical * ifrepeat)
{
	...
	( * ifrepeat)=false;
	#pragma cetus private(i, iel, iface, jface, nntemp, ntemp) 
	#pragma loop name check_refine#0 
	for (iel=0; iel<nelt; iel ++ )
	{
		...
		{
			/* check its six faces */
			#pragma cetus private(i, iface, jface, nntemp, ntemp) 
			#pragma loop name check_refine#0#0 
			for (i=0; i<6; i ++ )
			{
				jface=jjface[i];
				ntemp=sje[iel][i][0][0];
				...
				if (cbc[iel][i]==1)
				{
					...
					#pragma cetus private(iface, nntemp) 
					#pragma loop name check_refine#0#0#0 
				...
				else
				{
					if (cbc[iel][i]==2)
					{
						#pragma cetus firstprivate(ich) 
						#pragma cetus private(iface, nntemp) 
						#pragma cetus lastprivate(ich) 
						#pragma loop name check_refine#0#0#1 
						for (iface=0; iface<6; iface ++ )
						{
							if ((iface!=i)&&(iface!=jface))
							{
...
static void remap(double y[5][5][5], double y1[7][5][5][5], double x[5][5][5])
{
	...
	#pragma cetus private(i, ii, jj, kk) 
	#pragma loop name remap#0 
	#pragma cetus parallel 
	/*
	Disabled due to low profitability: #pragma omp parallel for private(i, ii, jj, kk)
	*/
	for (i=0; i<5; i ++ )
	{
		#pragma cetus private(ii, jj, kk) 
		#pragma loop name remap#0#0 
		for (kk=0; kk<5; kk ++ )
		{
			#pragma cetus private(ii, jj) 
			#pragma loop name remap#0#0#0 
			for (jj=0; jj<5; jj ++ )
			{
				#pragma cetus private(ii) 
				#pragma loop name remap#0#0#0#0 
				for (ii=0; ii<5; ii ++ )
				{
		...
		#pragma cetus private(ii, jj, kk) 
		#pragma loop name remap#0#1 
		for (kk=0; kk<5; kk ++ )
		{
			#pragma cetus private(ii, jj) 
			#pragma loop name remap#0#1#0 
			for (jj=0; jj<5; jj ++ )
			{
				#pragma cetus private(ii) 
				#pragma loop name remap#0#1#0#0 
				for (ii=0; ii<5; ii ++ )
				{
	...
	#pragma cetus private(ii, iz, jj, kk) 
	#pragma loop name remap#1 
	#pragma cetus parallel 
	/*
	Disabled due to low profitability: #pragma omp parallel for private(ii, iz, jj, kk)
	*/
	for (iz=0; iz<5; iz ++ )
	{
		#pragma cetus private(ii, jj, kk) 
		#pragma loop name remap#1#0 
		for (kk=0; kk<5; kk ++ )
		{
			#pragma cetus private(ii, jj) 
			#pragma loop name remap#1#0#0 
			for (jj=0; jj<5; jj ++ )
			{
				#pragma cetus private(ii) 
				#pragma loop name remap#1#0#0#0 
				for (ii=0; ii<5; ii ++ )
				{
...
static void merging(int iela[8])
{
	...
	*/
	...
	/* Normalized Loop */
	#pragma cetus lastprivate(i_1) 
	#pragma loop name merging#1 
	#pragma cetus parallel 
	/*
	Disabled due to low profitability: #pragma omp parallel for lastprivate(i_1)
	*/
	for (i_1=0; i_1<=3; i_1 ++ )
	{
		xc[ielnew][1+(2*i_1)]=x2;
	}
	i=(1+(2*i_1));
	#pragma cetus private(i) 
	#pragma loop name merging#2 
	#pragma cetus parallel 
	/*
	Disabled due to low profitability: #pragma omp parallel for private(i)
	*/
	...
	*/
	...
	{
		yc[ielnew][2+i_2]=y2;
		yc[ielnew][(2+i_2)+4]=y2;
	}
	i=(2+i_2);
	#pragma cetus private(i) 
	#pragma loop name merging#4 
	#pragma cetus parallel 
	/*
	Disabled due to low profitability: #pragma omp parallel for private(i)
	*/
	...
	*/
	...
	i=(4+i_3);
	/* update neighboring information */
	#pragma cetus firstprivate(ijel) 
	#pragma cetus private(cb, i, ielold, ii, jface, ntema, ntemp, ntempa) 
	#pragma cetus lastprivate(ijel) 
	#pragma loop name merging#6 
	for (i=0; i<6; i ++ )
	{
		jface=jjface[i];
		ielold=iela[children[i][0]];
		#pragma cetus private(ii) 
		#pragma loop name merging#6#0 
		#pragma cetus parallel 
		/*
		Disabled due to low profitability: #pragma omp parallel for private(ii)
		*/
		for (ii=0; ii<4; ii ++ )
		{
...
{
	int ix, iy, ip;
	#pragma cetus firstprivate(y) 
	#pragma cetus private(ip, ix, iy) 
	#pragma cetus lastprivate(y) 
	#pragma loop name remapz#0 
	#pragma cetus parallel 
	/*
	Disabled due to low profitability: #pragma omp parallel for private(ip, ix, iy) firstprivate(y) lastprivate(y)
	*/
	for (iy=0; iy<5; iy ++ )
	{
		#pragma cetus firstprivate(y) 
		#pragma cetus private(ip, ix) 
		#pragma cetus lastprivate(y) 
		#pragma loop name remapz#0#0 
		for (ix=0; ix<5; ix ++ )
		{
			y[0][iy][ix]=x1[0][iy][ix];
			y[1][iy][ix]=0.0;
			#pragma cetus private(ip) 
			#pragma loop name remapz#0#0#0 
			for (ip=0; ip<5; ip ++ )
			{
...
static void remapy(double x1[5][5][5], double x2[5][5][5], double y[5][5][5])
{
	int ix, iz, ip;
	#pragma cetus firstprivate(y) 
	#pragma cetus private(ip, ix, iz) 
	#pragma cetus lastprivate(y) 
	#pragma loop name remapy#0 
	#pragma cetus parallel 
	/*
	Disabled due to low profitability: #pragma omp parallel for private(ip, ix, iz) firstprivate(y) lastprivate(y)
	*/
	for (iz=0; iz<5; iz ++ )
	{
		#pragma cetus firstprivate(y) 
		#pragma cetus private(ip, ix) 
		#pragma cetus lastprivate(y) 
		#pragma loop name remapy#0#0 
		for (ix=0; ix<5; ix ++ )
		{
			y[iz][0][ix]=x1[iz][0][ix];
			y[iz][1][ix]=0.0;
			#pragma cetus private(ip) 
			#pragma loop name remapy#0#0#0 
			for (ip=0; ip<5; ip ++ )
			{
...
static void remapx(double x1[5][5][5], double x2[5][5][5], double y[5][5][5])
{
	int iy, iz, ip;
	#pragma cetus firstprivate(y) 
	#pragma cetus private(ip, iy, iz) 
	#pragma cetus lastprivate(y) 
	#pragma loop name remapx#0 
	#pragma cetus parallel 
	/*
	Disabled due to low profitability: #pragma omp parallel for private(ip, iy, iz) firstprivate(y) lastprivate(y)
	*/
	for (iz=0; iz<5; iz ++ )
	{
		#pragma cetus firstprivate(y) 
		#pragma cetus private(ip, iy) 
		#pragma cetus lastprivate(y) 
		#pragma loop name remapx#0#0 
		for (iy=0; iy<5; iy ++ )
		{
			y[iz][iy][0]=x1[iz][iy][0];
			y[iz][iy][1]=0.0;
			#pragma cetus private(ip) 
			#pragma loop name remapx#0#0#0 
			for (ip=0; ip<5; ip ++ )
			{
