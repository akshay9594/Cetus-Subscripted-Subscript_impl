...
public class RangeDomain implements Cloneable, Domain {
  /*==================================================================
    ...
    private static int debug =
            (Driver.getOptionValue("verbosity") != null)?
                    (Integer.valueOf(Driver.getOptionValue("verbosity")).intValue()): 0;
...
*/
  ...
  /*==================================================================
    ...
    * @param e the expression to be manipulated.
    * @return the resulting expression.
    */
    public Expression substituteForwardRange(Expression e) {
        Expression ret = e.clone();
        Set<Symbol> vars = SymbolTools.getAccessedSymbols(e);
        for (Symbol var : vars) {
            if (getRange(var) != null) {
                Expression forward = getForwardRange(var);
                if (!isOmega(ret)) {
                    ret = replaceSymbol(ret, var, forward);
                }
            }
        }
        return ret;
    ...
    /**
    * Performs {@link #substituteForwardRange(Expression)} on the value ranges
    * within the given range domain.
    * @param rd the range domain to be manipulated.
    * @return the resulting range domain.
    */
    public RangeDomain substituteForwardRange(RangeDomain rd) {
        RangeDomain ret = new RangeDomain();
        Set<Symbol> symbols = rd.getSymbols();
        for (Symbol symbol : symbols) {
            Expression range = rd.getRange(symbol);
            Expression subst = substituteForwardRange(range);
            if (isOmega(subst) || IRTools.containsSymbol(subst, symbol)) {
                ret.setRange(symbol, range.clone());
            } else {
