...
public class CFGraph extends DFAGraph {
    ...
    */
     ...
     * 
     * @param t a variable declarator 
     * @param arrayDecl a boolean for declarator with arrays
     */
        ...
        if (paramList != null) {
	        for (VariableDeclaration vd : paramList) {
	            if (vd.getDeclarator(0) instanceof NestedDeclarator) {
	                ...
	                addNode(node);
	                addEdge(last, node);
	            } else if (vd.getDeclarator(0) instanceof ProcedureDeclarator){
	                ProcedureDeclarator declarator = (ProcedureDeclarator) vd.getDeclarator(0);
	                DFANode node = new DFANode("stmt", declarator);
	                node.putData("ir", declarator);
	                node.putData("param", vd);
	                node.putData("param_idx", paramList.indexOf(vd));
